...
export type OrderItem = {
    ...
    quantity: number;
    unit_price: number;
    // Garantia (dias) desta peça nesta OS. Copiada do cadastro do produto ao
    // adicionar, editável antes de adicionar. Vazia/0 = usa a garantia padrão da OS.
...
export default function ServiceOrderItemsSection({ orderId, items, onChange, disabled, budgetApprovedAt, onApprovalReset }: Props) {
    ...
    // trigger, as a side effect of the item insert/delete that already ran by the
    ...
    // the tech can still override it below before adding.
    useEffect(() => {
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-2 mb-2">
                ...
                <div className="w-full sm:w-24">
                    ...
                    <input
                        ...
                        className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-green/50 bg-white text-sm text-center disabled:opacity-60 disabled:cursor-not-allowed disabled:bg-gray-50"
                    />
                </div>

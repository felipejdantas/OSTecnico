...
type Product = { id: string; name: string; unit: string; stock_quantity: number; sale_price: number };

type Props = {
    // Same pattern as the other item sections: with a quoteId, items are written
    // straight to the DB as soon as they're added/removed. Without one (new quote
    // not saved yet), items just live in local state until the quote is created.
    quoteId?: string;
    items: QuoteItem[];
...
export default function QuoteItemsSection({ quoteId, items, onChange, disabled }: Props) {
    ...
    const addItem = async () => {
        ...
        if (quoteId) {
            const { data, error } = await supabase
                .from('quote_items')
                .insert([{ quote_id: quoteId, ...newItem }])
                .select()
                .single();
            if (error) {
    ...
    return (
        <Card>
            ...
            {items.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-4">Nenhum produto adicionado a este orçamento</p>
            ) : (
